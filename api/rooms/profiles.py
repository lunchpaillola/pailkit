...
PROFILE_CONVERSATION: dict[str, Any] = deep_merge(BASE_CONFIG, {
    "capabilities": {
        "chat": True,
        "recording": False,
        "transcription": False,
...
PROFILE_AUDIO_ROOM: dict[str, Any] = deep_merge(BASE_CONFIG, {
    "media": {
    ...
    "interaction": {
        "prejoin": False  # Fast join for audio
    }
})
...
PROFILE_BROADCAST: dict[str, Any] = deep_merge(BASE_CONFIG, {
    "capabilities": {
        "chat": True,           # Q&A chat
...
PROFILE_PODCAST: dict[str, Any] = deep_merge(BASE_CONFIG, {
    "media": {
        "video": False,  # Audio only
        "audio": True,
        "screenshare_capable": False
